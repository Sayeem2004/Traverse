...
void solve() {
    ...
    int twon = 1<<n;
    dp.rsz(n, vll(twon, 1e18));
    FOR(i, 1, twon) {
        if (__builtin_popcount(i)==1) dp[__builtin_ctz(i)][i] = 0;
        else {
            F0R(j, n) if (i & (1<<j)) {
                int rest = i ^ (1<<j);
                F0R(k, n) if (rest & (1<<k)) {
                    dp[j][i] = min(dp[j][i], dp[k][rest] + t[k][j]);
                }
            }
        }
    }
    vll cmp1(n, 1e18), cmp2(n, 1e18);
    F0R(i, n) cmp1[i] = dp[i][twon-1];
    F0R(query, q) {
        cin >> c >> k;
        c--;
        vis.assign(n, false);
        ...
        while (! pq.empty()) {
            ...
            for (auto nb : adj[a]) {
                b = nb.F ; w = nb.S;
                if (! vis[nb.F] && dist[a] + w < dist[b]) {
                    dist[b] = dist[a]+w;
                    pq.insert(mp(dist[b], b));
                }
            }
        }
    }
